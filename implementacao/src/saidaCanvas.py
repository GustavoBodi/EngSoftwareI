...
class SaidaCanvas:
    ...
    def draw(self, color: str):
        ...
        self.__canvas.create_rectangle(x+25, y-90, x+self.__base+10, y-size-60, fill=color, tags=str(self.__posicao), outline="#6b0105", width=10)
    ...
    def get_checker_position(self) -> tuple[float, float]:
        if (not self.__reverse):
            return (self.__padding_x + self.__base / 2 + 25,
                    self.__padding_y -
                    self.__base / 2 -
